...
#include "../../../Utilities/md_model-steady.h"
// #include "../../../Utilities/normal_sample.h"
// #include "../../../Utilities/anomaly_sample.h"
...
int main(void) {
	...
	int MAX_MEASUREMENTS = 128; // Number of samples to keep in each axis
	int SAMPLE_RATE = 200;      // How fast to collect measurements (Hz)
	float MAD_SCALE = 1.4826;   // Scale MAD to be inline with py calcs
	float THRESHOLD = 100;    // Any MSE over this is an anomaly
...
//	buf_len = sprintf(buf, "normal MAD: x:%f, y:%f, z:%f\r\n", mad[0], mad[1], mad[2]);
//	HAL_UART_Transmit(&huart2, (uint8_t*) buf, buf_len, 100);
//	buf_len = sprintf(buf, "MAHA dist: %f\r\n", mahal);
//	HAL_UART_Transmit(&huart2, (uint8_t*) buf, buf_len, 100);
//
//	// For each axis, compute the MAD (scale up by 1.4826)
//	for (int axis = 0; axis < NUM_AXES; axis++) {
...
//
//	// Print out MAD calculations}
//	buf_len = sprintf(buf, "anomaly MAD: x:%f, y:%f, z:%f\r\n", mad[0], mad[1], mad[2]);
//	HAL_UART_Transmit(&huart2, (uint8_t*) buf, buf_len, 100);
//	buf_len = sprintf(buf, "MAHA dist: %f\r\n", mahal);
//	HAL_UART_Transmit(&huart2, (uint8_t*) buf, buf_len, 100);
	...
	while (1) {
		...
		while (i < MAX_MEASUREMENTS) {
			if (timestamp > 65535 - (1000 / SAMPLE_RATE)) {
				// Update timestamps to maintain sample rate
				timestamp = htim16.Instance->CNT;
			}
			if (htim16.Instance->CNT >= timestamp + (1000 / SAMPLE_RATE)) {
